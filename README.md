...
Базовый класс отвечает за хранение и логику работы с данными товаров и корзиной.\
...
страница, модалка
и корзина) для этого создано 3 дочерних класса -`CatalogProduct`,`ModalProduct` и `BasketProduct`.\
В базовом классе задаются элементы общие для всех видов (заголовок, цена) и соответствующие геттеры и сеттеры.
В дочерних добавляются элементы и геттеры и сеттеры для них, соответствующие необходимому шаблону.\
